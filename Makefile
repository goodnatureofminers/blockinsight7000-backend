...
BTC_HISTORY_ENV ?= $(LOCAL_DEPLOY_DIR)/envs/.env.btc-history-sync
...
run-btc-history-sync:
	@echo "Starting btc history sync with env file $(BTC_HISTORY_ENV)"
	@set -a; \
	source $(BTC_HISTORY_ENV); \
	set +a; \
	$(GO) run ./cmd/btc/history-sync
