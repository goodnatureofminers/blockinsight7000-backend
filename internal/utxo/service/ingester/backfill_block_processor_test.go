...
func Test_backfillBlockProcessor_processHeight(t *testing.T) {
	type testCase struct {
		name          string
		setupMocks    func(ctx context.Context, ctrl *gomock.Controller) (*backfillBlockProcessor, uint64)
		wantErrSubstr string
	}

	tests := []testCase{
		{
			name: "success writes block",
			setupMocks: func(ctx context.Context, ctrl *gomock.Controller) (*backfillBlockProcessor, uint64) {
				height := uint64(42)
				source := NewMockBackfillSource(ctrl)
				...
				source.EXPECT().
					...
					Return(&chain.BackfillBlock{
						...
						Inputs: []model.TransactionInput{{TxID: "tx1", Index: 0}},
					}, nil)
				writer.EXPECT().
		...
		{
			name: "fetch block error bubbles",
			setupMocks: func(ctx context.Context, ctrl *gomock.Controller) (*backfillBlockProcessor, uint64) {
				height := uint64(7)
				source := NewMockBackfillSource(ctrl)
				source.EXPECT().
		...
		{
			name: "write block error bubbles",
			setupMocks: func(ctx context.Context, ctrl *gomock.Controller) (*backfillBlockProcessor, uint64) {
				height := uint64(9)
				source := NewMockBackfillSource(ctrl)
				writer := NewMockBlockWriter(ctrl)
...
func Test_backfillBlockProcessor_Process(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		name          string
		prepare       func(ctrl *gomock.Controller) (*backfillBlockProcessor, args, *int)
		wantErrSubstr string
		wantCancel    bool
	}{
		{
		...
		},
		{
			...
			prepare: func(ctrl *gomock.Controller) (*backfillBlockProcessor, args, *int) {
				...
				source.EXPECT().
					...
					Return(&chain.BackfillBlock{Block: model.Block{Height: 5}}, nil)
				writer.EXPECT().
				...
				source.EXPECT().
					...
					AnyTimes().
					Return(&chain.BackfillBlock{Block: model.Block{Height: 6}}, nil)
				writer.EXPECT().
